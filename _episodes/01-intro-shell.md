...
In addition to the basic commands we'll cover in depth today, many tools for managing specific types of files, like [FFmpeg](https://ffmpeg.org/) for audiovisual media and [ImageMagick](https://www.imagemagick.org/script/index.php) for image files, are operated through the shell. Familiarity with the shell will allow you to use programs like these to make more complex changes to files in bulk, like converting file types.
